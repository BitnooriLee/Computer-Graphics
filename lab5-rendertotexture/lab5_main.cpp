...
void display()
{
	...
	drawScene(securityCamViewMatrix, securityCamProjectionMatrix); // draw scene 
	...
	///////////////////////////////////////////////////////////////////////////
	FboInfo &FB = fboList[1];
	glBindFramebuffer(GL_FRAMEBUFFER, FB.framebufferId); // to be replaced with another framebuffer when doing post processing
	glViewport(0, 0, w, h);
	...
	glClearColor(0.2f, 0.2f, 0.8f, 1.0f);
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	glActiveTexture(GL_TEXTURE0); // Active texture0;
	glBindTexture(GL_TEXTURE_2D, FB.colorTextureTarget); // Bind the framebuffer texture to texture unit 0.
